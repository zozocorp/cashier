            ...
            <div class="col-md-4 mt-40 pd-60">                
                ...
                @if($amount == 0)
                    <form id="checkoutForm" method="GET" action="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaystackController@paymentRedirect') }}">
                        <input type="hidden" name="redirect" value="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaystackController@changePlan', [
                    ...
                    function payWithPaystack() {
                        var handler = PaystackPop.setup({
                            ...
                            callback: function(response) {
                                ...
                                var url = '{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaystackController@changePlan', [
                                    'subscription_id' => $subscription->uid,
                                    'plan_id' => $newPlan->getBillableId(),
                                ]) }}&reference=' + reference;
