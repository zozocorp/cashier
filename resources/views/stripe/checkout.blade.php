    ...
    <body>
        <div class="main-container row mt-40">
            ...
            <div class="col-md-4 mt-40 pd-60">
                ...
                <img class="rounded" width="100%" src="{{ \Worker\Cashier\Cashier::public_url('/vendor/ema-cashier/image/stripe.png') }}" />
            </div>
            <div class="col-md-4 mt-40 pd-60">                
                ...
                @if ($service->getCardInformation($subscription->user) !== NULL)
                    ...
                    <div class="sub-section">
                        ...
                        <a href="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\StripeController@charge', [
                ...
                <form method="POST" action="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\StripeController@cancelNow', ['subscription_id' => $subscription->uid]) }}">
                    {{ csrf_field() }}
                ...
                <form id="stripe_button" style="display: none" action="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\StripeController@updateCard', [
                    '_token' => csrf_token(),
                    'subscription_id' => $subscription->uid,
                ]) }}" method="POST">
                    <input type="hidden" name="redirect" value="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\StripeController@charge', [
                        'subscription_id' => $subscription->uid,
                    ]) }}" />
                    ...
                    data-name="{{ \Worker\Model\Setting::get('site_name') }}"
                    data-email="{{ $subscription->user->getBillableEmail() }}"
                    data-description="{{ \Worker\Model\Setting::get('site_description') }}"
