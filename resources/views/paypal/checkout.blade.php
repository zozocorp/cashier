            ...
            <div class="col-md-4 mt-40 pd-60">
                ...
                <script>
                    var form = jQuery('<form>', {
                        'action': '{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaypalController@paymentRedirect') }}',
                        'target': '_top',
                        'method': 'GET'
                    }).append(jQuery('<input>', {
                        'name': '_token',
                    ...
                    paypal.Buttons({
                        ...
                        onApprove: function(data, actions) {
                            ...
                            return actions.order.capture().then(function(details) {
                                ...
                                form.append(jQuery('<input>', {
                                    'name': 'redirect',
                                    'value': '{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaypalController@checkout', $subscription->uid) }}',
                                    'type': 'hidden'
                                }));
                                form.submit();
                            });
                ...
                <form class="mt-5" method="POST" action="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaypalController@cancelNow', ['subscription_id' => $subscription->uid]) }}">
                    {{ csrf_field() }}
