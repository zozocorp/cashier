<html lang="en">
    <head>
        <title>{{ trans('cashier::messages.razorpay.renew_subscription') }}</title>
        ...
        <div class="row mt-40">
            ...
            <div class="col-md-4 mt-40 pd-60">
                ...
                <div class="text-center">
                    <img width="60%" src="{{ \Worker\Cashier\Cashier::public_url('/vendor/ema-cashier/image/razorpay.png') }}" />
                </div>
            </div>
            <div class="col-md-4 mt-40 pd-60">
                ...
                @if ($subscription->plan->price == 0)
                    <form method="POST" action="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\PaypalController@renew', ['subscription_id' => $subscription->uid]) }}">
                        {{ csrf_field() }}
                        <input type='hidden' name='plan_id' value='{{ $subscription->plan->getBillableId() }}' />
                        
                        <button
                            class="btn btn-primary mr-10 mr-2"
                        >{{ trans('cashier::messages.razorpay.renew_proceed') }}</button>
                ...
                @else
                    ...
                    <div class="hide" style="display:none">
                        <form action="{{ \Worker\Cashier\Cashier::lr_action('\Worker\Cashier\Controllers\RazorpayController@renew', [
                        '_token' => csrf_token(),
